...
- `src/cli/commands/generate.ts`:
  - `runGenerate(deps): Promise<number>` — logic thuần trả mã thoát, phụ thuộc tiêm:
    ...
    3. Bật → dựng `CodeAgent` (AI Gateway) + `existingSteps` (đọc step definition hiện có của app để ưu tiên tái dùng, BR-212); gọi `generateTestCase` (US-8.1).
    4. `{ ok: true }` → in đường dẫn file nháp; nhắc QC chạy thử + xác nhận qua mô tả hành vi + mở PR (ngoài nền tảng, US-8.3). `{ ok: false }` → in lý do, thoát khác 0.
  - `generateCommand(): Command` — bọc cho commander (ADR-017), `exitOverride()`; đăng ký ở `program.ts`.
- Nền tảng ghi file nháp vào `apps/<app-id>/` bằng thao tác file lúc chạy (không phải import `apps/`); `src/` không import `apps/` (coding-convention.md §Ranh-giới).

...
- `src/runner/cucumber-hooks.ts` — handler vòng đời do `wdio-service` (TICKET-017) đăng ký:
  - Trước khi phiên Appium mở: kiểm sự hiện diện các biến môi trường capability bắt buộc theo `CapabilityKind` — `AIMTAP_DEVICE_NAME`, `AIMTAP_PLATFORM_VERSION`, `AIMTAP_APP_PATH`, và với `'device'` thêm `AIMTAP_UDID`, `AIMTAP_XCODE_ORG_ID` (đúng bộ khóa `iosCapabilities` TICKET-017 đọc). Thiếu hoặc rỗng bất kỳ khóa nào thì ném `PlatformFailure` liệt kê khóa thiếu, dừng lượt chạy sớm bằng thông báo đọc được thay vì lỗi cấp giao thức Appium (ADR-009). Đây là `PlatformFailure`, không thành test case hỏng (ADR-016). Việc *populate* các biến này do CLI ráp lúc khởi chạy (US-4.3) từ `AppConfig` + `DeviceContext` đã validate (FR-DEV-02) + secret ký mã (US-1.2) — `DeviceContext` một mình không mang `deviceName`/đường dẫn build/secret nên không đủ làm nguồn; hook ở đây chỉ là guard hiện diện.
...
- [ ] Thiếu bất kỳ `AIMTAP_*` bắt buộc theo `CapabilityKind` → guard ném `PlatformFailure` liệt kê khóa thiếu, không mở phiên, không thành test case hỏng (test đơn vị với env giả lập).
- [ ] Guard chạy trước khi tiêm sink; env đầy đủ thì sink được tiêm và không ném.

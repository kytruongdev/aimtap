...
Không có mục đang mở (`open-items.md` "Đang mở" trống). Các mục đã giải: ma trận phụ thuộc (ADR-014), mô hình Test Runner (ADR-013), vị trí wait-policy (ADR-015), discriminant loại lỗi (ADR-016), chữ ký async Evidence Collector (interface-spec §Evidence Collector). Còn treo — quyết định thư viện CLI (Product Owner/SA), cần trước US-4.2.

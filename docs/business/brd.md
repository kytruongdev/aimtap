...
| Tầng | Tên | Nội dung | Ví dụ |
|---|---|---|---|
| 1 | Test suite | Toàn bộ test case của một ứng dụng. | — |
| 2 | Test feature | Một luồng nghiệp vụ của ứng dụng. Là nhóm chứa các test case. | Login, Signup |
| 3 | **Test case** | Một trường hợp cụ thể với một kết quả mong đợi. Đây là đơn vị mang trạng thái đạt hoặc hỏng. | "Login with wrong password shows error" |
| 4 | Bước (step) | Một câu mô tả hành vi bên trong một test case. | "Enter an invalid password" |

Một test case kiểm tra đúng một hành vi và chạy độc lập được với các test case khác.

**Test case** gồm hai phần: phần mô tả hành vi bằng ngôn ngữ tự nhiên theo góc nhìn người dùng, và phần cài đặt thực thi từng câu mô tả. Phần mô tả hành vi là thứ được thực thi, không phải tài liệu đi kèm.

**Bằng chứng thực thi** của một test case gồm ba thứ: trạng thái kết quả, nhật ký các bước đã chạy kèm kết quả từng bước, và ảnh chụp màn hình tại bước hỏng.

**Lượt chạy (run)** là một lần thực thi test suite, hoặc một tập con test case được chọn từ nó. Tập con được chọn để chạy gọi là "tập chạy".
...
| BO-02 | Giảm chi phí bảo trì test case khi giao diện ứng dụng thay đổi. |
| BO-03 | Rút ngắn thời gian soạn một test case mới. |
...
|---|---|---|
| QC | Người dùng chính. Soạn test case, chuẩn bị thiết bị, chạy test suite, đọc báo cáo, cập nhật kết quả lên Jira. | Chạy được hồi quy mà không thao tác tay từng bước; khi một test case hỏng thì biết hỏng ở màn hình nào, bước nào. |
| Reviewer | Dev hoặc QC Lead. Rà soát test case trong pull request và phê duyệt trước khi merge vào nhánh chính. | Đọc được thay đổi của một pull request và hiểu test case kiểm tra điều gì. |
| QC Lead | Người chịu trách nhiệm chất lượng đầu ra kiểm thử. Quyết định phạm vi test case, tiêu thụ lớp phân tích. Có thể đồng thời là reviewer. | Nắm được độ phủ hồi quy, tỷ lệ vượt qua theo thời gian, test case nào thiếu ổn định. |
...
flowchart LR
    ...
    system -->|gọi sinh test case, tìm lại locator| claude
    claude -->|test case, locator đề xuất| system
...
3. Soạn test case cho các luồng chính của ứng dụng, nhóm theo test feature.
...
3. Chọn ứng dụng, chạy toàn bộ test suite của nó, và khởi chạy lượt chạy.
4. Nền tảng thực thi từng test case, ghi nhật ký các bước đã chạy, ghi trạng thái kết quả, và chụp ảnh màn hình tại bước hỏng của các test case hỏng.
...
### QC — soạn một test case mới (Phase 1)
...
3. Viết phần mô tả hành vi của test case bằng ngôn ngữ tự nhiên, đặt trong test feature tương ứng.
4. Cài đặt phần thực thi cho những câu mô tả chưa có sẵn.
5. Chạy thử test case trên thiết bị và điều chỉnh cho tới khi kết quả ổn định.
6. Tạo pull request.
...
### Reviewer — phê duyệt một test case (Phase 1 trở đi)
1. Nhận pull request chứa test case mới hoặc test case đã sửa.
...
2. Đọc nhật ký thực thi để biết test case đã đi qua những bước nào và hỏng ở bước nào.
...
2. Xem tỷ lệ vượt qua theo thời gian, danh sách màn hình hay hỏng, danh sách test case thiếu ổn định.
3. Quyết định điều chỉnh phạm vi test case hoặc ưu tiên sửa lỗi.
...
- Soạn test case thủ công: mô tả hành vi bằng ngôn ngữ tự nhiên và phần cài đặt bằng WebdriverIO chạy trên Appium, locator tổ chức theo Page Object, test case nhóm theo test feature.
...
- Đưa test case vào nhánh chính mà không qua pull request được phê duyệt, kể cả test case do AI sinh.
- Máy chủ kết quả tập trung và tổng hợp dữ liệu xuyên nhiều máy QC.
- Tích hợp vào quy trình CI/CD.
- Dịch vụ thiết bị trên cloud.
- AI tự phê duyệt test case hoặc tự kết luận chất lượng.
...
| BC-08 | Mọi thay đổi test case và Page Object vào nhánh chính đi qua pull request được rà soát và phê duyệt, không phân biệt do người hay AI tạo ra. Quy trình rà soát do product owner kiểm soát, nằm ngoài phạm vi nền tảng. |
...
| Mã | Chỉ số | Đo ở tầng | Đo từ |
|---|---|---|---|
| SM-01 | Số luồng nghiệp vụ được phủ bởi test case tự động. | Test feature | Phase 1 |
| SM-02 | Thời gian thực hiện một vòng hồi quy tự động. | Lượt chạy | Phase 1 |
| SM-03 | Tỷ lệ lượt chạy cho kết quả lặp lại (không dao động do nguyên nhân ngoài lỗi ứng dụng). | Lượt chạy | Phase 1 |
| SM-04 | Thời gian soạn một test case mới, so sánh trước và sau khi có hỗ trợ AI. | Test case | Phase 2 |
| SM-05 | Số lần tự phục hồi thành công so với số lần locator hỏng. | Test case | Phase 2 |
| SM-06 | Tỷ lệ pull request chứa test case do AI sinh được phê duyệt mà không phải sửa phần cài đặt. | Pull request | Phase 2 |
...
|---|---|
| WebdriverIO là framework tự động hóa cho ứng dụng web và di động; Appium là framework mã nguồn mở cho ứng dụng di động qua giao thức WebDriver. Page Object Model áp dụng cho di động dưới dạng screen object, tách test case khỏi locator. | [WebdriverIO Appium Tutorial](https://www.testmuai.com/learning-hub/webdriverio-appium/), [Mobile Test Automation using WebDriver.io and Appium](https://spurqlabs.com/mobile-test-automation-using-webdriver-io-and-appium/) |
| Phân cấp test feature và test case: một tệp mô tả một luồng nghiệp vụ, bên trong là nhiều trường hợp cụ thể; mỗi trường hợp kiểm tra một hành vi và chạy độc lập được. | [Solving: How to organise feature files? — Cucumber](https://cucumber.io/blog/bdd/solving-how-to-organise-feature-files/), [Cucumber Best Practices](https://www.testmuai.com/blog/cucumber-best-practices/) |
| Cơ chế self-healing: chặn lỗi không tìm thấy phần tử, so sánh trạng thái giao diện hiện tại với dữ liệu locator lịch sử, đề xuất locator thay thế lúc chạy. Rủi ro đi kèm là bám nhầm phần tử tương đương làm test case báo đạt trong khi ứng dụng đã hỏng. | [Healenium – Self-healing locator tool](https://blog.nashtechglobal.com/healenium-self-healing-locator-tool-for-automation-test/), [The 6 Types of AI Self-Healing in Test Automation](https://www.qawolf.com/blog/self-healing-test-automation-types), [Self-Healing Test Automation 2026 Guide](https://qaskills.sh/blog/self-healing-test-automation-2026-guide) |

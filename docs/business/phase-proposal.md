...
Toàn bộ giá trị của hai phase sau đều bám vào tầng thực thi của phase này. AI ở Phase 2 không tạo ra một luồng chạy mới — nó bọc quanh hai điểm đã tồn tại trong Phase 1: bước soạn kịch bản (sinh kịch bản) và bước tìm phần tử lúc chạy (tự phục hồi locator). Nếu chưa có kịch bản chạy được và chưa có Page Object để đặt locator vào, thì không có chỗ để gắn AI. Lớp phân tích ở Phase 3 đọc dữ liệu do Phase 1 ghi ra; dữ liệu chỉ có giá trị khi đã tích lũy qua nhiều lượt chạy, nên Phase 1 phải chạy thật một khoảng thời gian trước khi Phase 3 có gì để phân tích.
...
- EP-17 (rà soát và phê duyệt kịch bản qua pull request) — quy trình phê duyệt phải vận hành ổn định trước khi AI bắt đầu sinh kịch bản ở Phase 2, vì đó là cơ chế duy nhất chặn kịch bản sai đi vào nhánh chính.
...
**Mục tiêu:** Người soạn kịch bản không cần biết lập trình; thời gian soạn kịch bản mới và chi phí bảo trì kịch bản khi giao diện thay đổi đều giảm. Quyền chấp nhận kịch bản vẫn thuộc về con người, thực hiện qua phê duyệt pull request.

**Phụ thuộc vào:** Phase 1 — cấu trúc Page Object (EP-02) làm nơi đặt locator được sinh ra và được sửa; tầng thực thi (EP-05) làm nơi chèn bước tự phục hồi khi không tìm thấy phần tử; cơ chế ghi bản ghi kết quả (EP-19) làm nơi ghi nhận mỗi lần tự phục hồi; quy trình phê duyệt pull request (EP-17) làm cơ chế kiểm soát kịch bản do AI sinh.

**Vì sao không làm sớm hơn:**
Hai chức năng AI đều cần một đầu vào chỉ có sau khi Phase 1 chạy thật. Sinh kịch bản cần một tập câu mô tả hành vi và step definition đã được chấp nhận để kịch bản sinh ra bám theo. Tự phục hồi cần biết locator nào đang dùng và hỏng theo kiểu nào; nếu chưa có kịch bản chạy đủ nhiều thì chưa xác định được cơ chế tự phục hồi có giải quyết đúng vấn đề hay không.
...
**Mục tiêu:** QC Lead trả lời được các câu hỏi về xu hướng chất lượng từ dữ liệu đã tích lũy, làm cơ sở điều chỉnh phạm vi kịch bản và ưu tiên sửa lỗi.
...
**Vì sao làm sau cùng:**
Giá trị của lớp phân tích tỷ lệ thuận với lượng dữ liệu đã tích lũy. Xác định kịch bản thiếu ổn định dựa trên việc trạng thái đổi qua lại giữa nhiều lượt chạy liên tiếp; với ít lượt chạy, kết luận không có cơ sở. Xây lớp này trước khi có dữ liệu tạo ra một giao diện rỗng, không dùng được.

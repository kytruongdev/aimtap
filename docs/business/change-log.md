...
| 2026-08-22 | **Sinh test case: bỏ đầu vào page source dump tay, AI tự lái app khám phá lấy locator (PO định hướng).** QC chỉ cung cấp mô tả bằng lời (DESC); AI viết `.feature` từ mô tả, tự lái một phiên thiết bị sống đi theo kịch bản và inspect từng màn để lấy locator, rồi viết phần cài đặt. Thêm tiền điều kiện UC-203 (app đã cài + phiên thiết bị sống; dữ liệu test có sẵn) và luồng ngoại lệ E3 (AI khám phá không đi hết kịch bản). Phiên khám phá dùng dữ liệu test của app (kể cả nhánh bí mật) để tiến bước; mở rộng giả định rủi ro dữ liệu (AS-02, `srs.md` §3) cho bề mặt lộ dữ liệu rộng hơn — PO chấp nhận cho Phase 2. Không đổi: AI viết `.feature`, mô hình run tất định, tự phục hồi (ADR-024), QC review + chạy-xanh + tự mở PR. | `brd.md` (§5 dòng giá trị, §6 happy path, §7 phạm vi, AS-02, §5 bảng tham chiếu +1 dòng), `epic-map.md` EP-11, `phase-2/` (UC-203, US-206, FR-GEN-01, §3 giả định, BR-211 + state diagram) | Có. SA viết ADR (Proposed) — generate = agentic (AI cầm công cụ MCP điều khiển thiết bị); heal giữ ADR-024; reconcile ADR-025 (seam CodeAgent nhận hai kiểu: phiên agent có MCP cho generate, `invoke` một-lần cho heal). Nhánh generate (US-8.x) đã hiện thực trên mô hình DESC+PAGE → SA đánh giá lại tác động. |
